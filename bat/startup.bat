...
REM set "JAVA_OPTS=%JAVA_OPTS% -XX:+UseCMSCompactAtFullCollection"
...
set "COBAR_HOME=%cd%"
cd %CURR_DIR%
if exist "%COBAR_HOME%\conf\log4j.xml" goto okHome
echo ---------------------------------------------------
echo Error: COBAR_HOME environment variable is not defined correctly. COBAR_HOME: %COBAR_HOME%
echo ---------------------------------------------------
goto end
...
set "COBAR_CLASSPATH= -jar %COBAR_HOME%\GoodCobar.jar"
...
set "RUN_CMD=%RUN_CMD% %COBAR_CLASSPATH%"
call %RUN_CMD%
